#include "stdafx.h"
#include "..\Public\Item_GndSword.h"
#include "GameInstance.h"
#include "HierarchyNode.h"
#include "Model.h"

CItem_GndSword::CItem_GndSword(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
	: CGameObject(pDevice, pDeviceContext)
{
}

CItem_GndSword::CItem_GndSword(const CItem_GndSword & rhs)
	: CGameObject(rhs)
...
HRESULT CItem_GndSword::NativeConstruct_Prototype(const CTransform::TRANSFORMDESC& TransformDesc)
{
...
HRESULT CItem_GndSword::NativeConstruct(void * pArg)
{
	if (nullptr == pArg)
		return E_FAIL;
...
void CItem_GndSword::Tick(_float fTimeDelta)
{
	m_pOBB->Update(m_pTransformCom->Get_WorldMatrix());
}

void CItem_GndSword::Late_Tick(_float fTimeDelta)
{
...
HRESULT CItem_GndSword::SetUp_BonePtr()
{
...
HRESULT CItem_GndSword::SetUp_Components()
{
	/* For.Com_Renderer */
	...
	///* For.Com_Model*/
	//if (FAILED(__super::Add_Components(LEVEL_GAMEPLAY, TEXT("Prototype_Component_Model_Item_Blade"), TEXT("Com_Model"), (CComponent**)&m_pModelCom)))
	//	return E_FAIL;
...
HRESULT CItem_GndSword::SetUp_ConstantTable()
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
CItem_GndSword * CItem_GndSword::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext, const CTransform::TRANSFORMDESC& TransformDesc)
{
	CItem_GndSword*		pInstance = new CItem_GndSword(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype(TransformDesc)))
	{
		MSGBOX(TEXT("Failed to Created : CItem_GndSword"));
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CItem_GndSword::Clone(void * pArg)
{
	CItem_GndSword*		pInstance = new CItem_GndSword(*this);

	if (FAILED(pInstance->NativeConstruct(pArg)))
	{
		MSGBOX(TEXT("Failed to Cloned : CItem_GndSword"));
		Safe_Release(pInstance);
	}
	return pInstance;
}

void CItem_GndSword::Free()
{
	__super::Free();
