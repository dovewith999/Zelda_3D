#include "stdafx.h"
#include "..\Public\Item_SwordHandle.h"
#include "GameInstance.h"
#include "HierarchyNode.h"
#include "Model.h"
#include "Player.h"

CItem_SwordHandle::CItem_SwordHandle(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
	: CGameObject(pDevice, pDeviceContext)
{
}

CItem_SwordHandle::CItem_SwordHandle(const CItem_SwordHandle & rhs)
	: CGameObject(rhs)
...
HRESULT CItem_SwordHandle::NativeConstruct_Prototype(const CTransform::TRANSFORMDESC& TransformDesc)
{
...
HRESULT CItem_SwordHandle::NativeConstruct(void * pArg)
{
	if (nullptr == pArg)
		return E_FAIL;
	...
	m_pTransformCom->Set_Scaled(XMVectorSet(0.007f, 0.007f, 0.007f, 1.f));
...
void CItem_SwordHandle::Tick(_float fTimeDelta)
{
...
void CItem_SwordHandle::Late_Tick(_float fTimeDelta)
{
...
HRESULT CItem_SwordHandle::Render()
{
...
HRESULT CItem_SwordHandle::SetUp_BonePtr()
{
...
HRESULT CItem_SwordHandle::SetUp_Components()
{
	/* For.Com_Renderer */
	...
	/* For.Com_Model*/
	if (FAILED(__super::Add_Components(LEVEL_PLAY_STATIC, TEXT("Prototype_Component_Model_Item_SwordHandle"), TEXT("Com_Model"), (CComponent**)&m_pModelCom)))
		return E_FAIL;

	return S_OK;
}

HRESULT CItem_SwordHandle::SetUp_ConstantTable()
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
CItem_SwordHandle * CItem_SwordHandle::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext, const CTransform::TRANSFORMDESC& TransformDesc)
{
	CItem_SwordHandle*		pInstance = new CItem_SwordHandle(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype(TransformDesc)))
	{
		MSGBOX(TEXT("Failed to Created : CItem_SwordHandle"));
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CItem_SwordHandle::Clone(void * pArg)
{
	CItem_SwordHandle*		pInstance = new CItem_SwordHandle(*this);

	if (FAILED(pInstance->NativeConstruct(pArg)))
	{
		MSGBOX(TEXT("Failed to Cloned : CItem_SwordHandle"));
		Safe_Release(pInstance);
	}
	return pInstance;
}

void CItem_SwordHandle::Free()
{
	__super::Free();
