...
CItem_Bow::CItem_Bow(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
	: CGameObject(pDevice, pDeviceContext)
{
}

CItem_Bow::CItem_Bow(const CItem_Bow & rhs)
	: CGameObject(rhs)
...
HRESULT CItem_Bow::NativeConstruct_Prototype(const CTransform::TRANSFORMDESC& TransformDesc)
{
...
HRESULT CItem_Bow::NativeConstruct(void * pArg)
{
	if (nullptr == pArg)
		return E_FAIL;
	...
	m_pTransformCom->Rotation(XMVector3Normalize(m_pTransformCom->Get_State(CTransform::STATE_UP)), XMConvertToRadians(180.f));

	m_pTransformCom->Set_Scaled(XMVectorSet(0.03f, 0.03f, 0.03f, 1.f));

	return S_OK;
}

void CItem_Bow::Tick(_float fTimeDelta)
{
}

void CItem_Bow::Late_Tick(_float fTimeDelta)
{
...
HRESULT CItem_Bow::SetUp_BonePtr()
{
...
HRESULT CItem_Bow::SetUp_Components()
{
	/* For.Com_Renderer */
...
HRESULT CItem_Bow::SetUp_ConstantTable()
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
CItem_Bow * CItem_Bow::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext, const CTransform::TRANSFORMDESC& TransformDesc)
{
	CItem_Bow*		pInstance = new CItem_Bow(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype(TransformDesc)))
	{
		MSGBOX(TEXT("Failed to Created : CItem_Bow"));
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CItem_Bow::Clone(void * pArg)
{
	CItem_Bow*		pInstance = new CItem_Bow(*this);

	if (FAILED(pInstance->NativeConstruct(pArg)))
	{
		MSGBOX(TEXT("Failed to Cloned : CItem_Bow"));
		Safe_Release(pInstance);
	}
	return pInstance;
}
