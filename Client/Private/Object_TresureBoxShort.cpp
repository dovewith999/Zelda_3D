#include "stdafx.h"
#include "..\Public\Object_TresureBoxShort.h"
#include "GameInstance.h"
#include "Player.h"

CObject_TresureBoxShort::CObject_TresureBoxShort(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
	: CGameObject(pDevice, pDeviceContext)
{
}

CObject_TresureBoxShort::CObject_TresureBoxShort(const CObject_TresureBoxShort & rhs)
	: CGameObject(rhs)
{

}

HRESULT CObject_TresureBoxShort::NativeConstruct_Prototype(const CTransform::TRANSFORMDESC& TransformDesc)
{
...
HRESULT CObject_TresureBoxShort::NativeConstruct(void * pArg)
{
...
void CObject_TresureBoxShort::Tick(_float fTimeDelta)
{
	CGameInstance* pGameInstance = CGameInstance::Get_Instance();
	Safe_AddRef(pGameInstance);
	...
	case STATE_OPEN_SHORT:
		...
		if (0 == m_fTimeAcc)
		{
			pGameInstance->PlaySoundW(L"Box_Open_Short.wav", SOUND_EFFECT, 0.2f);
...
void CObject_TresureBoxShort::Late_Tick(_float fTimeDelta)
{
	...
	if (3.f > Get_Instance())
	{
		m_pTarget_Collider = dynamic_cast<CCollider*>(pGameInstance->Get_ComponentPtr(LEVEL_DUNGEON, TEXT("Layer_Player"), TEXT("Com_OBB")));
		Safe_AddRef(m_pTarget_Collider);

		if (nullptr != m_pTarget_Collider)
		{
			if (m_pOBB->Collision_OBB(m_pTarget_Collider, CCollider::TYPE_OBB))
			{

				if (STATE_IDLE == m_eState)
				{
					m_pPlayer->Check_Front(static_cast<_double>(fTimeDelta), true, 0.f, 0.f);
					m_pPlayer->Set_OpenBox_Short(true);
				}
			...
			{
				m_pPlayer->Set_OpenBox_Short(false);
			}

			if (CPlayer::PLAYERSTATE::PLAYER_GETITEM_SHORT == m_pPlayer->Get_State())
			{
				if (STATE_IDLE == m_eState)
				{
					m_pPlayer->Set_Position(m_pTransformCom->Front_Pos());
					m_pPlayer->Set_LookAt(m_pTransformCom->Get_State(CTransform::STATE_POSITION));
					m_eState = STATE_OPEN_SHORT;
				}
			}
...
HRESULT CObject_TresureBoxShort::Render()
{
...
HRESULT CObject_TresureBoxShort::SetUp_ConstantTable()
{
...
CObject_TresureBoxShort * CObject_TresureBoxShort::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext, const CTransform::TRANSFORMDESC& TransformDesc)
{
	CObject_TresureBoxShort*		pInstance = new CObject_TresureBoxShort(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype(TransformDesc)))
	{
		MSGBOX(TEXT("Failed to Created : CObject_TresureBoxShort"));
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CObject_TresureBoxShort::Clone(void * pArg)
{
	CObject_TresureBoxShort*		pInstance = new CObject_TresureBoxShort(*this);

	if (FAILED(pInstance->NativeConstruct(pArg)))
	{
		MSGBOX(TEXT("Failed to Cloned : CObject_TresureBoxShort"));
		Safe_Release(pInstance);
	}
	return pInstance;
}

void CObject_TresureBoxShort::Free()
{
	__super::Free();
