...
CItem_Shield::CItem_Shield(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
	: CGameObject(pDevice, pDeviceContext)
{
}

CItem_Shield::CItem_Shield(const CItem_Shield & rhs)
	: CGameObject(rhs)
...
HRESULT CItem_Shield::NativeConstruct_Prototype(const CTransform::TRANSFORMDESC& TransformDesc)
{
...
HRESULT CItem_Shield::NativeConstruct(void * pArg)
{
	if (nullptr == pArg)
		return E_FAIL;
...
void CItem_Shield::Tick(_float fTimeDelta)
{
	...
	{
		m_ItemDesc.pBoneName = "cl_cl_RhandA                   ";
		SetUp_BonePtr();
	}
}

void CItem_Shield::Late_Tick(_float fTimeDelta)
{
...
HRESULT CItem_Shield::SetUp_BonePtr()
{
...
HRESULT CItem_Shield::SetUp_Components()
{
	/* For.Com_Renderer */
	...
	/* For.Com_Model*/
	if (FAILED(__super::Add_Components(LEVEL_PLAY_STATIC, TEXT("Prototype_Component_Model_Item_Shield"), TEXT("Com_Model"), (CComponent**)&m_pModelCom)))
		return E_FAIL;

	return S_OK;
}

HRESULT CItem_Shield::SetUp_ConstantTable()
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
CItem_Shield * CItem_Shield::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext, const CTransform::TRANSFORMDESC& TransformDesc)
{
	CItem_Shield*		pInstance = new CItem_Shield(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype(TransformDesc)))
	{
		MSGBOX(TEXT("Failed to Created : CItem_Shield"));
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CItem_Shield::Clone(void * pArg)
{
	CItem_Shield*		pInstance = new CItem_Shield(*this);

	if (FAILED(pInstance->NativeConstruct(pArg)))
	{
		MSGBOX(TEXT("Failed to Cloned : CItem_Shield"));
		Safe_Release(pInstance);
	}
	return pInstance;
}
