#include "stdafx.h"
#include "..\Public\UI_PlayerHp.h"
#include "GameInstance.h"

CUI_PlayerHp::CUI_PlayerHp(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
...
HRESULT CUI_PlayerHp::NativeConstruct_Prototype(const CTransform::TRANSFORMDESC & TransformDesc)
{
...
HRESULT CUI_PlayerHp::NativeConstruct(void * pArg)
{
...
void CUI_PlayerHp::Late_Tick(_float fTimeDelta)
{
...
HRESULT CUI_PlayerHp::SetUp_Components()
{
	/* For.Com_Renderer */
	...
	/* For.Com_Texture */
	if (FAILED(__super::Add_Components(LEVEL_PLAY_STATIC, TEXT("Prototype_Component_Texture_UI_PlayerHp"), TEXT("Com_Texture"), (CComponent**)&m_pTextureCom)))
		return E_FAIL;

	return S_OK;
}

CUI_PlayerHp * CUI_PlayerHp::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext, const CTransform::TRANSFORMDESC & TransformDesc)
{
	CUI_PlayerHp* pInstance = new CUI_PlayerHp(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype(TransformDesc)))
	{
		MSGBOX(TEXT("Error : Failed Create UI_PlayerHp from UI_Player.cpp"));
...
CGameObject * CUI_PlayerHp::Clone(void * pArg)
{
	...
	{
		MSGBOX(TEXT("Error : Failed Cloned UI_PlayerHp from UI_Player.cpp"));
		Safe_Release(pInstance);
	}
	return pInstance;
}
