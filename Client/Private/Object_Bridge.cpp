#include "stdafx.h"
#include "..\Public\Object_Bridge.h"
#include "GameInstance.h"

CObject_Bridge::CObject_Bridge(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
...
HRESULT CObject_Bridge::NativeConstruct_Prototype(const CTransform::TRANSFORMDESC& TransformDesc)
{
...
HRESULT CObject_Bridge::NativeConstruct(void * pArg)
{
...
void CObject_Bridge::Tick(_float fTimeDelta)
{
...
HRESULT CObject_Bridge::SetUp_Components()
{
	/* For.Com_Renderer */
	...
	/* For.Com_Model*/
	if (FAILED(__super::Add_Components(LEVEL_DUNGEON, TEXT("Prototype_Component_Model_Object_Bridge"), TEXT("Com_Model"), (CComponent**)&m_pModelCom)))
		return E_FAIL;

	return S_OK;
}

HRESULT CObject_Bridge::SetUp_ConstantTable()
{
...
CObject_Bridge * CObject_Bridge::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext, const CTransform::TRANSFORMDESC& TransformDesc)
{
	CObject_Bridge*		pInstance = new CObject_Bridge(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype(TransformDesc)))
	{
		MSGBOX(TEXT("Failed to Created : CObject_Bridge"));
...
CGameObject * CObject_Bridge::Clone(void * pArg)
{
	...
	{
		MSGBOX(TEXT("Failed to Cloned : CObject_Bridge"));
		Safe_Release(pInstance);
	}
	return pInstance;
}
